...
class abstract_arena:
    ...
    def get_full_rankings(self, filename = "full_rankings", batch_size = 512):
        ...
        allow_train_recommendations = getattr(self.args, "allow_train_recommendations", False)
        if(self.data.nodrop):
            if ground_truth_split == "test":
                dump_sources = [self.data.valid_user_list] if allow_train_recommendations else [self.data.train_nodrop_user_list, self.data.valid_user_list]
            else:
                dump_sources = [self.data.test_user_list] if allow_train_recommendations else [self.data.train_nodrop_user_list, self.data.test_user_list]
        else:
            if ground_truth_split == "test":
                dump_sources = [self.data.valid_user_list] if allow_train_recommendations else [self.data.train_user_list, self.data.valid_user_list]
            else:
                dump_sources = [self.data.test_user_list] if allow_train_recommendations else [self.data.train_user_list, self.data.test_user_list]
        dump_dict = merge_user_list(dump_sources)
        if allow_train_recommendations:
            print("Ablation mode: training interactions are not masked from rankings.")
